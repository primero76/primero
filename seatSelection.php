...
$cleardb_url = parse_url(getenv("CLEARDB_DATABASE_URL"));
    ...
    $detID = $_SESSION['detailsID'];
    $trackingID = $_SESSION['tracking'];
    $numofpassengers = $_SESSION['passengerCount'];
    $depart = $_SESSION['depFrom'];
    $dest = $_SESSION['depTo'];
    $flightN = $_SESSION['flyName'];
    $depDate = $_SESSION['depDate1'];
    $depTime = $_SESSION['depTime1'];
    $seatName = "";
    if ($_SERVER["REQUEST_METHOD"] == "POST")
    {
        $ins = "insert into seats2 (trackingId, flightName, departure, destination,departureDate) VALUES ('$trackingID','$flightN','$depart','$dest','$depDate')";
        mysqli_query($conn,$ins);
        $seatName = $_POST["seat"];
        foreach ($seatName as $sn => $value)
        {
            $insquery = "insert into seats(seatNo,trackingId) VALUES ('$value','$trackingID')";
...
<script>
    <?php
        $sql  = "select * from seats where trackingId = (SELECT trackingId FROM seats2 where flightName='$flightN' and departure='$depart' and destination='$dest' and departureDate = '$depDate')";
